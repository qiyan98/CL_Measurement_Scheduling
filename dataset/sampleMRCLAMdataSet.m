...
save MRCLAMdata.mat
%% get rid of those measurements with respect to known landmark
% barcode_robots = Barcodes(1:n_robots,2);
% for i = 1:n_robots
%     X = eval(['Robot',num2str(i),'_Measurement']);
%     for j = 1:length(X)
%         if isempty(intersect(X(j,2),barcode_robots)) % the robot identity not included
%             X(j,2) = n_robots + 1;
%         else
%             X(j,2) = Barcodes(Barcodes(:,2) == X(j,2),1);
%         end
%     end
%     X(X(:,2) > n_robots,:) = [];
%     eval(['Robot',num2str(i),'_Measurement = X;']);
% end